...
const ArtikelList = (props) => {
    const { showDetail, artikel, message, update, deleteArtikel } = props;

    let allResult = artikel.map((data) =>
        <tr key={data.id_artikel}>
            <td>{data.id_user}</td>
            ...
            <td>
                ...
                <button className="btn-update" onClick={() => update(data.id_user, data.title, data.description, data.date, data.id_artikel)}><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pencil" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
